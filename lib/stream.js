...
var AjaxStream = function( env ){
    ...
    this.loadOrMakeConfig = function( filePath, callback ){
        var store = new Storage( filePath );
        var config = {
            def_env: "local",
            local: {
        ...
        store.load( null, function( config ){
            if( Object.getOwnPropertyNames( config ).length == 0 ) store.push( config, function( config ){
                if( config.def_env ) console.log( "cudatel-ajax.json file created." );
            });
    ...
    this.boot = function( env ){
        $this.loadOrMakeConfig( "./cudatel-ajax", function( config ){
            $this.config = config;
